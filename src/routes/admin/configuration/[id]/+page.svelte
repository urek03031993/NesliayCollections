<script lang="ts">
    ...
    $effect.pre(()=>{
        if(form?.success) {
            toastStore.success("Configuración editada satisfactoriamente");
            goto(resolve('/admin/configuration'));
        }

        if(form?.errors) {
            toastStore.error("Ocurrio un error al editar la configuración revise por favor");
        }
    });
</script>
...
<main class="pt-24 lg:pl-72 pb-20 px-6 lg:px-12 min-h-screen">
    <div class="max-w-5xl mx-auto">
        ...
        <form name="configurationEditForm" method="POST" use:enhance data-netlify="true">
            <div class="space-y-1 mt-5">
                ...
                <input class="w-full bg-surface-container-low border-none rounded-lg p-4 pl-8 focus:ring-2 focus:ring-primary/20 transition-all font-body text-black"
                        type="text" name="key" id="key" bind:value={ data.key } placeholder="Configuration Key" required />
            ...
            <div class="space-y-1 mt-5">
                ...
                <input class="w-full bg-surface-container-low border-none rounded-lg p-4 pl-8 focus:ring-2 focus:ring-primary/20 transition-all font-body text-black"
                        type="text" name="value" id="value" bind:value={ data.value } placeholder="Configuration Value" required />
